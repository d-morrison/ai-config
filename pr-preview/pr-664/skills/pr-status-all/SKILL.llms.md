...
>     ```
>
>     **`--json reviews` returns the full review history, not one entry per reviewer, and a reviewer’s *decisive* state persists across neutral comments** – GitHub only clears `CHANGES_REQUESTED` when that same reviewer later `APPROVED`s, or via an explicit dismissal; a neutral `COMMENTED` review in between does **not** clear it. Filter to only `APPROVED`/`CHANGES_REQUESTED` states *before* reducing to each author’s latest review – reducing over all states first lets a later `COMMENTED` round hide an earlier `CHANGES_REQUESTED` (verified with a synthetic fixture: naive reduction incorrectly cleared it, state-filtered reduction correctly kept it blocking). Any non-empty result **blocks** regardless of what any bot says – only the human (or an explicit dismissal) resolves it. Return the reviewer login(s) from the array, not just a count.
>
