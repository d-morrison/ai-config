...
Phase 1 can still delegate sidecar work directly via the `Agent` tool within a single PR’s own ARDI round — see `ardia`’s “Lightweight sidecar delegation” note, including its guidance on giving a judgment-heavy sidecar task a stronger model. Phase 2 has no sidecar-delegation angle of its own, per the non-decomposable note above.
