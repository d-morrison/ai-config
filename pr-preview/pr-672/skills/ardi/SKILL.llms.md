...
For a judgment-heavy sidecar task (a subtle root-cause hunt, adjudicating a deadlocked rebuttal before escalating to a human), give the subagent a stronger model via the `Agent` tool’s `model` parameter (e.g. `model: 'opus'`) — see [`select-model`](../../skills/select-model/SKILL.llms.md)’s decision tree for when the bump is warranted.
