...
5.  **Commit & push** the change so it persists. What that means depends on where step 2 wrote it:
    ...
    - **Project’s own repo** (the normal project-specific case) — commit, push a branch, and open a PR there, same as any other change to that repo.
    - **`~/.claude/projects/*/memory/` staging** (only when the project repo has no agent-doc infrastructure yet) — no git commit; it persists locally only. Tell the user this is temporary staging and a PR adding agent-doc infrastructure to that repo is still needed.
