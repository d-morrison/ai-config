...
Did I discover a repo convention for a repo **we own** that has checked-in agent docs? → put it IN that repo (its `CLAUDE.md`, `.github/agents/*.md`, `.github/instructions/*.md`, or `.github/copilot-instructions.md`), via a PR, so the whole team and every `@claude` session there sees it. Do NOT keep repo-specific notes in ai-config (`memories/repo/` is retired). For a repo without agent-doc infrastructure, fall back to that repo’s local Claude project memory: `~/.claude/projects/<project-path>/memory/` (write directly; no commit).
