...
- **Still requires fully-clean.** Never merge a PR that isn’t fully clean (`fully-clean.md`: every CI workflow green and finished, latest review has zero outstanding findings) just because `mwc` is active — it removes the “ask first” step, not the CI-green / review-clean bar. Merging while any CI check is still `IN_PROGRESS`, `QUEUED`, or `PENDING`, or before review checks finalize, is strictly forbidden.
