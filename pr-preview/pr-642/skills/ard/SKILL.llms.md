...
- **Findings sourced from a different PR than the target branch** — treat the cited review as the finding source, but apply fixes on the target PR branch the user names. Post the ARD summary on the target PR, then reply on each original inline thread with the target commit SHA and target PR link so the provenance stays auditable across both threads.
