...
- **The proxy allows branch creation/push but BLOCKS branch deletion.** Pushing a
  *new* branch (even one other than the harness-assigned `claude/...`) works, but a
  ...
  ("send-pack: unexpected disconnect" / "remote end hung up", then "Everything
  up-to-date" with a non-zero exit). So a throwaway branch (e.g. a push-capability
...
- **The `@claude` agent can run a parallel session that posts a phantom commit SHA.**
  ...
  that **never reached the remote** (e.g. "Addressed in `a841fc7` --- a parallel session's
  `<sha>` didn't land"). The fixes are really there via *your* pushed commit; the cited SHA
