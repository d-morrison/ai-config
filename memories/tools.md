...
- **ARDI step 2: fetch review comments via REST API, not just `gh pr view --json comments`.** When reading the latest review to check for new findings, use `gh api repos/<o>/<r>/pulls/<N>/comments --paginate` to fetch ALL comments — both top-level PR comments AND inline review comments. `gh pr view --json comments` can return an incomplete list and miss suppressed (low-confidence) inline comments or non-suppressed comments that were added after the PR view output was generated. Always use the REST endpoint to get the authoritative full list of review findings before deciding what to ARD. (Discovered on ai-config#645: multiple inline comments on specific lines were not visible in `gh pr view`'s summary; the REST endpoint revealed them, and a non-suppressed comment matching a specific comment ID was found there.)
...
- **Markdown table formatting (MD055/MD056).** Table rows must stay on single lines to preserve pipe alignment. Wrapping a table cell across multiple lines breaks `|` alignment and triggers violations. Solution: rewrite cell content concisely on a single line rather than word-wrapping. Prefer clarity (short, complete descriptions) over line-wrapping to hit a length target. (Learned on ai-config#645 while fixing lint-qmd prose violations.)
- **Code fence language tags for non-shell commands (MD040).** Do NOT tag CLI commands that aren't shell-executable with `bash` or `sh`. Claude slash commands like `/ums`, `/plugin`, `/also`, and other application-level directives are not shell commands. Tag them with `text` or a neutral tag instead. MD040 requires a language spec, but mislabeling creates a false implication that the command is runnable as shell (fails for readers who try it). (Discovered on ai-config#645 when /ums and /plugin blocks were initially tagged as `bash`/`sh` and flagged by review.)
