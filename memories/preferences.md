...
- **Delegating investigation doesn't mean re-delegating facts already gathered.**
  When the coordinator already holds a research result --- from an earlier agent's
  output or work done earlier in the same session --- pass that context into the
  next agent's brief rather than asking a fresh agent to rediscover it.
  Re-delegation wastes budget and risks a worse answer than the one already in
  hand.
  The "delegate investigation" rule (above) targets root-cause work the
  coordinator hasn't done yet; it doesn't say to throw away findings and start
  over when context already resolves the question.
