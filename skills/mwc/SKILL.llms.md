...
- **Still requires fully-clean.** Never merge a PR that isn’t fully clean (`fully-clean.md`: every CI workflow green, latest review has zero outstanding findings) just because `mwc` is active — it removes the “ask first” step, not the CI-green / review-clean bar.
