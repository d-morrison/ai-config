...
`CHANGES_REQUESTED` states *before* reducing to each author's latest
review is required -- reducing over *all* states first would let a later
`COMMENTED` round hide an earlier `CHANGES_REQUESTED` (verified with a
synthetic fixture: a `CHANGES_REQUESTED` followed by a `COMMENTED` from
the same author incorrectly produced no output under the naive reduction,
and correctly still blocked once state-filtered first).
