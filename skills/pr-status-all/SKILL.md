...
> 3. **Human formal reviews -- read-only, don't request one.**
>    Human reviewers who use GitHub's review UI appear in the formal reviews
>    list, separate from the `@claude` comment (Signal 1) and the Copilot
>    review (Signal 2). Their reviews carry different weight than bot reviews:
>    a `CHANGES_REQUESTED` from a human is **blocking** regardless of what any
>    bot review says (only the human or an explicit dismissal clears it), and
>    an `APPROVED` at the current head counts as a clean external verdict, the
>    same weight as a clean Copilot review. Fetch reviews from non-bot humans
>    at the current head:
>    ```bash
>    set -o pipefail
>    head="$(gh pr view <N> --json headRefOid -q .headRefOid)"
...
>    - `{"state":"APPROVED", ...}` -- human approved; counts as a clean
>      external verdict if threads (Signal 5) are resolved. Report as
>      `approved:<reviewer>`.
>    - `{"state":"CHANGES_REQUESTED", ...}` -- **blocking**. Report as
>      `CHANGES_REQUESTED:<reviewer>` even if CI is green and bot review is
>      clean.
>    - `{"state":"COMMENTED", ...}` -- comment-only review; not a verdict
>      either way. Report as `commented:<reviewer>`.
>    Do not request a review here -- that's `ardi`'s job.
> 4. **CI state** -- `gh pr checks <N>` (`PR_CHECKS`); name any failing/pending
...
>    treat as unresolved.
> 6. **Behind main?** -- fetch the head ref too (a fresh subagent has no local
...
> / `no verdict at head` / `CHANGES_REQUESTED:<reviewer>` / `approved:<reviewer>`),
> threads (`resolved` / `N open` / `N+ open (cap)`), behind-main (`up to date`
> / `N commits`).
...
- **Review** -- `clean`, `N open` (with the headline finding), `none found`
  ...
  still going **or** the latest review predates the latest commit (per
  subagent item 1's currency check) -- either way, the current head hasn't
  been reviewed yet.
- **External** -- synthesizes Signal 2 (Copilot) and Signal 3 (human formal
  reviews): `clean` if either a Copilot review shows zero findings or a human
  APPROVED at the current head (with threads resolved); `CHANGES_REQUESTED:<reviewer>`
  if any human has a blocking review at the current head (overrides any bot
  all-clear -- a bot's "ready for merge" does not clear a human's block);
  `N open` if Copilot found findings; or `no verdict at head` if neither
  Copilot nor any human has reviewed at the current commit. Always check both
  signals -- a clean Copilot review coexisting with a human `CHANGES_REQUESTED`
  is NOT clean. This step is read-only and doesn't request a review, so it
  can't tell "never asked" from "unreachable" -- report the plain evidence,
...
**fully clean** -- at least one of Review (`@claude`) or External (Copilot or
human APPROVED) is `clean` at the current head AND no human `CHANGES_REQUESTED`
is outstanding (a human's block is not cleared by a bot's all-clear) AND all CI
workflows are green AND it's not behind main AND every inline review thread is
resolved (the only open conversation being the final all-clear and your reply).
If both Review and External come back `none found` / `no verdict at head`, this
skill has no evidence of a genuine external verdict at all -- report the PR as
not confirmed clean and point at `ardi` to obtain one; don't guess whether a
self-review already covers it. Never hedge with "ready except for one nit."
